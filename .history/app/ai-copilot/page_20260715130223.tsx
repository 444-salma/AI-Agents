...
const PROMPTS = [
  {
    icon: Brain,
    label: "Summarize this customer",
    q: "Summarize this customer.",
  },
  {
    icon: TrendingUp,
    label: "Explain the financial analysis",
    q: "Explain the financial analysis for this customer.",
  },
  {
    icon: FileText,
    label: "Which documents are missing?",
    q: "Which documents are missing or need renewal?",
  },
  {
    icon: Shield,
    label: "Why is this recommendation generated?",
    q: "Why was the primary recommendation generated? Explain the reasoning.",
  },
  {
    icon: BookOpen,
    label: "Explain this policy",
    q: "Explain the most relevant banking policy for this customer right now.",
  },
  {
    icon: Sparkles,
    label: "Show KYC status",
    q: "What is the KYC status for this customer?",
  },
];
...
function buildAnswer(
  ...
  if (
    ...
    if (lower.includes("kyc")) {
      const kycDoc = comp.checklist.find((d) =>
        d.documentType.toLowerCase().includes("kyc"),
      );
      if (kycDoc) {
    ...
    return `The most relevant policy issue right now:\n\n${
      relevant.length > 0
        ? relevant
            ...
            .map(
              (d) =>
                `**${d.documentType}** (${d.status}): ${d.policyExplanation}`,
            )
            .join("\n\n")
  ...
  if (
    ...
    const prods = ci.customerProfile.filter(
      (p) =>
        p.label === "Segment" ||
        p.label === "Annual Revenue" ||
        p.label === "Relationship Manager",
    );
...
export default function AICopilotPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      const ciResult = runCompanyIntelligenceAgent(
        co,
        (p.data || []) as BankingProduct[],
        (t.data || []) as TimelineEvent[],
      );
      const faResult = runFinancialAnalysisAgent(
        co,
        (s.data || []) as FinancialStatement[],
      );
      const compResult = runComplianceAgent(
        co,
        (d.data || []) as CompanyDocument[],
      );
      const nbaResult = runNextBestActionAgent(
        co,
        ciResult,
        faResult,
        compResult,
  ...
  function sendMessage(text: string) {
    if (!text.trim() || !ready || !company || !ci || !fa || !comp || !nba)
      return;
  ...
  function renderContent(content: string) {
    return content.split("\n").map((line, i) => {
      ...
      if (line.startsWith("• ")) {
        return (
          <p key={i} className="pl-3 leading-relaxed text-foreground/85">
            {line}
          </p>
        );
      }
        ...
        {!hasMessages ? (
          <div className="flex flex-col items-center justify-center h-full max-w-2xl mx-auto text-center">
            ...
            <p className="text-sm text-muted-foreground mb-8 max-w-md leading-relaxed">
              Four AI agents have analyzed this customer's profile, financials,
              documents, and compliance. Ask a question or pick a prompt below.
            </p>
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 p-3 rounded-2xl border bg-card shadow-sm">
            <input
              ...
              onKeyDown={handleKey}
              placeholder={
                ready ? "Ask about this customer..." : "Loading agents..."
              }
              disabled={!ready}
          ...
          <p className="text-xs text-muted-foreground text-center mt-2">
            Decision support only · All final decisions require authorized
            banking professional approval
          </p>
        </div>
      </div>
    </div>
