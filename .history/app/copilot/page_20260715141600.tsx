...
function generateResponse(
  ...
  if (/summarize|who is|tell me about|overview|profile/.test(lower)) {
    return {
      text: ci.executiveSummary,
      sources: ["Company Profile", "Banking Products", "Relationship History"],
    };
  }
  if (
    /what should|next action|recommend|priority|what do i do|next step/.test(
      lower,
    )
  ) {
    const extras = nba.additionalActions
      .slice(0, 2)
      .map((a) => `• **${a.action}** (${a.priority}): ${a.reason}`)
      .join("\n");
    return {
  ...
  if (/missing|document|compliance|expired|doc/.test(lower)) {
    ...
    if (needsUpdate.length > 0)
      text += `**Require Renewal:**\n${needsUpdate.map((d) => `• ${d.documentType}: ${d.policyExplanation}`).join("\n")}`;
    if (missing.length === 0 && needsUpdate.length === 0)
      text =
        "All required documents are complete and up to date. No action required at this time.";
    return {
      text: text.trim(),
      sources: [
        "Document Registry",
        "Compliance & Document Agent",
        "Policy Engine BP-003, BP-004, BP-005",
      ],
    };
  }
  if (
    /financial|finance|revenue|profit|margin|income|health score|balance|liquidity/.test(
      lower,
    )
  ) {
  ...
  if (/kyc|know your customer|aml|anti-money/.test(lower)) {
    ...
    const kycItems = policyEngine.kycRequirements.mandatoryInformation
      .slice(0, 5)
      .map((i) => `• ${i}`)
      .join("\n");
    return {
  ...
  if (/policy|rule|regulation|procedure/.test(lower)) {
    const summary = policyEngine.bankingPolicies
      .map((p) => `• **${p.title}** (${p.id}): ${p.description}`)
      .join("\n");
    return {
      text: `Here are the active banking policies that apply to this customer:\n\n${summary}`,
      sources: ["Policy Engine — All Active Policies"],
    };
  }
  if (/credit|facility|loan|financing/.test(lower)) {
    const policy = policyEngine.bankingPolicies.find((p) => p.id === "BP-001");
    const procedure = policyEngine.operationalProcedures.find(
      (p) => p.id === "OP-002",
    );
    const steps =
    ...
    return {
      ...
      sources: [
        "Policy Engine — BP-001 Credit Risk",
        "Operational Procedure OP-002",
      ],
    };
  }
  if (/risk|rating/.test(lower)) {
    ...
    return {
      ...
      sources: [
        "Policy Engine — BP-006 Risk Management",
        "Financial Analysis Agent",
        "Company Profile",
      ],
    };
...
export default function CopilotPage() {
  ...
  useEffect(() => {
    ...
    const ci = runCompanyIntelligenceAgent(company, products, timeline);

    const fa = runFinancialAnalysisAgent(company, statements);

    const comp = runComplianceAgent(company, documents);

    const nba = runNextBestActionAgent(company, ci, fa, comp, products);
      ...
      <div className="flex items-center justify-center min-h-screen bg-background">
        <div className="text-center space-y-4 animate-fade-in">
          ...
          <p className="text-sm font-semibold text-slate-600 dark:text-slate-400">
            Getting customer information ready...
          </p>
        </div>
      </div>
    );
        ...
        <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
          ...
          {messages.map((msg, idx) => (
            ...
            >
              ...
              <div
                className={`flex-1 max-w-2xl space-y-2 ${msg.role === "user" ? "items-end flex flex-col" : ""}`}
              >
                <div
                ...
                {msg.role === "assistant" &&
                  ...
                  msg.sources.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 px-1">
                      <span className="text-[10px] font-bold text-slate-300 dark:text-slate-600 uppercase tracking-wide">
                        Based on
                      </span>
                      {msg.sources.map((src) => (
                        <span
                          key={src}
                          className="text-[10px] font-medium bg-slate-100/80 dark:bg-white/[0.05] text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-full border border-slate-200/60 dark:border-white/[0.06]"
                        >
                          {src}
                        </span>
                      ))}
      ...
      <div className="border-t border-slate-200/80 dark:border-white/[0.06] bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl px-4 py-4">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="text-[10px] text-slate-300 dark:text-slate-600 text-center mt-2 font-medium">
            This assistant helps you prepare — all final decisions require your
            approval.
          </p>
        </div>
      </div>
