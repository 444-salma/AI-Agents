  ...
  return (
    <div className="animate-slide-up mx-4 sm:mx-6 mt-5 mb-1">
      <div className="relative overflow-hidden rounded-2xl px-5 py-4 flex items-start gap-4"
        style={{ background: 'linear-gradient(135deg, rgba(16,185,129,0.12), rgba(5,150,105,0.08))', border: '1px solid rgba(16,185,129,0.25)' }}>
        {/* Glow */}
        <div className="absolute right-0 top-0 w-40 h-full opacity-30 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at right, rgba(16,185,129,0.3), transparent 70%)' }} />
...
export default function Dashboard() {
  ...
  const policySummary = comp.complianceScore >= 80
  ...
  const pBadge = priorityBadge[nba.primaryRecommendation.priority] || priorityBadge.Medium;
      ...
      <div className="relative overflow-hidden">
        <div className="ai-hero-gradient px-6 py-10 lg:px-10 lg:py-12">
          ...
          <div className="absolute top-0 right-0 w-96 h-96 opacity-30 pointer-events-none"
            style={{ background: 'radial-gradient(circle at 70% 30%, rgba(139,92,246,0.5), transparent 60%)' }} />
          <div className="absolute bottom-0 left-1/4 w-64 h-64 opacity-20 pointer-events-none"
            style={{ background: 'radial-gradient(circle, rgba(59,130,246,0.6), transparent 60%)' }} />
          ...
          <div className="relative max-w-4xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div className="space-y-3 animate-slide-up">
              ...
              <div className="flex flex-wrap items-center gap-4 pt-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-white/80">{company.name}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-white/15 flex items-center justify-center">
            ...
            <div className="hidden sm:flex items-center justify-center animate-ai-float shrink-0">
              <div className="relative w-36 h-36">
                <div className="absolute inset-0 rounded-3xl opacity-30 animate-spin-slow"
                  style={{ background: 'conic-gradient(from 0deg, #3b82f6, #8b5cf6, #3b82f6)', padding: '2px' }}>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  {[0, 1, 2, 3].map(i => (
                    <div key={i} className="absolute w-3 h-3 rounded-full bg-blue-300 opacity-70"
                      style={{ animation: `orbit ${2 + i * 0.5}s linear infinite ${i * 90}deg`, transformOrigin: 'center' }} />
                  ))}
                </div>
              </div>
            </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {statusCards.map(({ icon: Icon, label, value, sub, gradient, bg, border, text }, i) => (
              <div key={label}
                className={`card-lift rounded-2xl border p-4 ${bg} ${border} space-y-3 animate-slide-up`}
        ...
        <section>
          ...
          <div className="glass-card-blue rounded-3xl p-6 border relative overflow-hidden">
            ...
            <div className="absolute inset-0 opacity-30 pointer-events-none"
              style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.04) 0%, rgba(99,102,241,0.04) 100%)' }} />
                ...
                <div>
                  ...
                  {!typingDone && summaryVisible && (
                    <div className="flex gap-1 mt-0.5">
                      ...
                      <span className="w-1 h-1 rounded-full bg-blue-500 typing-dot-2" />
                      <span className="w-1 h-1 rounded-full bg-blue-500 typing-dot-3" />
                    </div>
                  )}
                </div>
              </div>

              <div className={`transition-all duration-700 ${typingDone ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
                <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed mb-4">
                  {ci.executiveSummary.split('.').slice(0, 3).join('.') + '.'}
                </p>
                <div className="flex flex-wrap gap-2">
                  {ci.keyInsights.slice(0, 3).map((insight, i) => (
                    <div key={i}
                      className="text-xs text-blue-700 dark:text-blue-300 bg-blue-100/70 dark:bg-blue-900/30 border border-blue-200/60 dark:border-blue-800/40 px-3 py-1.5 rounded-full font-medium leading-tight max-w-xs">
                      {insight.split('.')[0] + '.'}
                    </div>
                  ))}
                </div>
            ...
            {aiFindings.map(({ num, title, insight, color, dotColor }, i) => (
              <div key={num}
                ...
                <div className="flex items-start gap-3 mb-3">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-extrabold ${color} tracking-widest`}>{num}</span>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed pl-8">{insight}</p>
              </div>
            ))}
          </div>
        </section>
                ...
                return (
                  <div key={label} className="flex lg:flex-col items-center flex-1 gap-0">
                    <div className="flex lg:flex-col items-center w-full gap-0">
                      {/* Connector before */}
                      {i > 0 && (
                        <div className={`hidden lg:block h-px flex-1 transition-all duration-500 ${active ? 'pipeline-connector bg-gradient-to-r from-blue-400 to-violet-500' : 'bg-slate-200 dark:bg-white/10'}`}
                          style={{ height: '2px', transitionDelay: `${i * 350}ms` }} />
                      )}
                      {i > 0 && (
                        <div className={`lg:hidden w-px h-5 transition-all duration-500 ${active ? 'bg-gradient-to-b from-blue-400 to-violet-500' : 'bg-slate-200 dark:bg-white/10'}`}
                          style={{ transitionDelay: `${i * 350}ms` }} />
                      )}

                      {/* Step circle */}
                      <div className={`relative w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 transition-all duration-500 cursor-default hover:scale-110 ${
                        active
                          ? `bg-gradient-to-br ${color} shadow-md`
                          : 'bg-slate-100 dark:bg-white/[0.06] border border-slate-200 dark:border-white/[0.08]'
                      }`}
                        style={{ transitionDelay: `${i * 350}ms` }}>
                        <Icon className={`w-5 h-5 transition-colors duration-500 ${active ? 'text-white' : 'text-slate-400 dark:text-slate-600'}`} />
                        {active && (
                          <div className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${color} opacity-30 animate-ping`} />
                        )}
                      </div>

                      {/* Connector after */}
                      {i < workflowSteps.length - 1 && (
                        <div className={`hidden lg:block h-px flex-1 transition-all duration-500 ${pipelineStep > i + 1 ? 'bg-gradient-to-r from-violet-400 to-blue-500' : 'bg-slate-200 dark:bg-white/10'}`}
                          style={{ height: '2px', transitionDelay: `${(i + 0.5) * 350}ms` }} />
                      )}
                      {i < workflowSteps.length - 1 && (
                        <div className={`lg:hidden w-px h-5 transition-all duration-500 ${pipelineStep > i + 1 ? 'bg-gradient-to-b from-violet-400 to-blue-500' : 'bg-slate-200 dark:bg-white/10'}`}
                          style={{ transitionDelay: `${(i + 0.5) * 350}ms` }} />
                      )}
                    </div>

                    <div className={`lg:text-center px-1 py-2 lg:py-3 space-y-0.5 transition-all duration-500 ${active ? 'opacity-100' : 'opacity-40'}`}
                      style={{ transitionDelay: `${i * 350}ms` }}>
                      <p className="text-xs font-bold text-slate-800 dark:text-slate-100 leading-tight">{label}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 leading-tight hidden lg:block">{sub}</p>
                    </div>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-4 py-3.5">
      <span className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest w-28 shrink-0 pt-0.5">{label}</span>
