...
export function runCompanyIntelligenceAgent(
  ...
  if (!company) {
    return {
      executiveSummary:
        "No company selected. Please search and select a corporate customer to start the analysis.",
      customerProfile: [],
      keyInsights: ["Please select a customer before running the analysis."],
      relationshipHighlights: [],
    };
  }
  const activeProducts = products.filter((p) => p.status === "Active");
  const totalExposure = products.reduce(
    (s, p) => s + (p.outstanding_amount || 0),
    0,
  );
  const years =
    new Date().getFullYear() -
    new Date(company.relationship_since).getFullYear();
  ...
  const customerProfile = [
    ...
    {
      label: "Client Since",
      value: new Date(company.relationship_since).getFullYear().toString(),
    },
  ];
  ...
  if (years >= 5)
    keyInsights.push(
      `Long-standing ${years}-year banking relationship indicating strong loyalty and trust.`,
    );
  if (company.risk_rating === "Low")
    keyInsights.push(
      "Low risk classification reflects strong financial position and consistent repayment history.",
    );
  if (activeProducts.length >= 3)
    keyInsights.push(
      `Multi-product relationship with ${activeProducts.length} active products demonstrates deep banking engagement.`,
    );
  if (totalExposure > 0)
    keyInsights.push(
      `Total bank exposure of ${fmt(totalExposure)} represents a significant contribution to the portfolio.`,
    );
  keyInsights.push(
    `The ${company.sector} sector aligns with Saudi Vision 2030 economic priorities, supporting relationship strategic importance.`,
  );
  ...
  if (expiringProducts.length > 0) {
    keyInsights.push(
      `${expiringProducts.length} product(s) approaching expiry — renewal planning should begin promptly.`,
    );
  }

  const relationshipHighlights = timeline
    .slice(0, 4)
    .map(
      (e) =>
...
export function runFinancialAnalysisAgent(
  ...
  const revenueGrowth = prior
    ? ((latest.total_revenue - prior.total_revenue) / prior.total_revenue) * 100
    : null;
  const netIncomeGrowth = prior
    ? ((latest.net_income - prior.net_income) / prior.net_income) * 100
    : null;
  ...
  if (latest.current_ratio >= 1.5)
    strengths.push(
      `Strong liquidity with current ratio of ${latest.current_ratio.toFixed(2)}x, indicating solid short-term solvency.`,
    );
  else if (latest.current_ratio < 1.2)
    weaknesses.push(
      `Current ratio of ${latest.current_ratio.toFixed(2)}x is below the 1.5x threshold, suggesting liquidity pressure.`,
    );

  if (latest.net_margin >= 10)
    strengths.push(
      `Healthy net profit margin of ${latest.net_margin.toFixed(1)}%, demonstrating effective cost management.`,
    );
  else if (latest.net_margin < 5)
    weaknesses.push(
      `Net profit margin of ${latest.net_margin.toFixed(1)}% is below benchmarks, indicating cost pressures.`,
    );

  if (latest.return_on_equity >= 15)
    strengths.push(
      `Return on equity of ${latest.return_on_equity.toFixed(1)}% reflects strong shareholder value creation.`,
    );
  if (latest.operating_cash_flow > 0)
    strengths.push(
      `Positive operating cash flow of ${fmt(latest.operating_cash_flow)} confirms strong internal cash generation.`,
    );

  if (revenueGrowth !== null) {
    if (revenueGrowth >= 10)
      strengths.push(
        `Revenue grew ${revenueGrowth.toFixed(1)}% year-on-year, demonstrating strong business momentum.`,
      );
    else if (revenueGrowth < 0)
      weaknesses.push(
        `Revenue declined ${Math.abs(revenueGrowth).toFixed(1)}% versus prior year — warrants further investigation.`,
      );
    else
      observations.push(
        `Revenue grew ${revenueGrowth.toFixed(1)}% year-on-year, reflecting stable performance.`,
      );
  }

  if (latest.debt_to_equity > 2.5)
    weaknesses.push(
      `Debt-to-equity of ${latest.debt_to_equity.toFixed(2)}x indicates high leverage, limiting future borrowing capacity.`,
    );
  else if (latest.debt_to_equity <= 1.5)
    strengths.push(
      `Conservative leverage at ${latest.debt_to_equity.toFixed(2)}x debt-to-equity provides headroom for additional financing.`,
    );

  observations.push(
    `Total assets: ${fmt(latest.total_assets)} | Equity: ${fmt(latest.total_equity)} | Gross margin: ${latest.gross_margin.toFixed(1)}%.`,
  );
  if (netIncomeGrowth !== null)
    observations.push(
      `Net income ${netIncomeGrowth >= 0 ? "increased" : "decreased"} by ${Math.abs(netIncomeGrowth).toFixed(1)}% year-on-year.`,
    );
...
export function runComplianceAgent(
  ...
  const explanations: Record<string, Record<string, string>> = {
    Complete: {
      default:
        "Document is valid, current, and on file as required by internal policy.",
    },
    "Needs Update": {
      "Commercial Registration":
        "Per internal document policy (BP-003), the Commercial Registration is expiring within 90 days and must be renewed before any credit facility renewal or account changes can proceed.",
      "Articles of Association":
        "The Articles of Association require updating to reflect current ownership structure per internal onboarding requirements.",
      "National ID":
        "One or more authorized signatory National IDs have expired. Per KYC policy, valid IDs must be provided before processing any new requests.",
      "KYC Form":
        "Annual KYC refresh is overdue. Per AML policy (BP-002), KYC must be refreshed annually for all corporate customers.",
      default:
        "Document requires renewal or update per internal policy before proceeding.",
    },
    Missing: {
      "UBO Declaration":
        "UBO Declaration has not been submitted. Per AML/CFT policy (BP-004), all beneficial owners holding 25%+ must be disclosed before any banking relationship can proceed.",
      "Financial Statements":
        "Audited financial statements are not on file. Per credit policy (BP-005), latest statements (not older than 18 months) are mandatory for all credit facilities.",
      "KYC Form":
        "Corporate KYC form is missing. Per AML policy, this form is mandatory for all corporate customers before any facility processing.",
      default:
        "Document is not on file. This is a mandatory requirement that must be fulfilled before proceeding.",
    },
  };
  ...
  const checklist = documents.map((doc) => {
    ...
    if (doc.status === "Complete") {
      policyExplanation =
        "Document is valid, current, and on file per internal requirements.";
    } else {
  ...
  const score =
    checklist.length > 0 ? Math.round((complete / checklist.length) * 100) : 0;
  const overallStatus: ComplianceOutput["overallStatus"] =
    missing > 0
      ? "Non-Compliant"
      : needsUpdate > 0
        ? "Needs Attention"
...
export function runNextBestActionAgent(
  ...
  for (const doc of comp.checklist.filter((d) => d.status === "Missing")) {
    actions.push({
      ...
      priority: "Critical",
      businessImpact:
        "Regulatory compliance requirement. Failure to obtain may result in relationship suspension and regulatory exposure.",
    });
  }
  ...
  for (const doc of comp.checklist.filter((d) => d.status === "Needs Update")) {
    actions.push({
      ...
      priority: "High",
      businessImpact:
        "Required to maintain operational continuity and prevent credit facility processing blockage.",
    });
  }
  ...
  for (const p of products.filter((p) => {
    if (
      !p.expiry_date ||
      !["Credit Facility", "Term Loan", "Letter of Guarantee"].includes(
        p.product_type,
      )
    )
      return false;
    ...
    actions.push({
      ...
      businessImpact:
        "Prevents facility expiry and ensures uninterrupted financing availability for customer operations.",
    });
  }
  ...
  if (fa.healthScore >= 75 && comp.overallStatus === "Compliant") {
    actions.push({
      ...
      priority: "Medium",
      businessImpact:
        "Opportunity to expand banking products and increase wallet share with a high-quality customer.",
    });
  }
  ...
  if (fa.weaknesses.length > 0) {
    actions.push({
      ...
      priority: "Medium",
      businessImpact:
        "Proactive risk management — understanding challenges enables early intervention.",
    });
  }
  ...
  actions.push({
    ...
    reason:
      "Maintaining current customer records ensures informed decision-making and audit readiness.",
    priority: "Low",
    businessImpact:
      "Improves relationship quality, supports audit readiness, and ensures data accuracy.",
  });
  ...
  const primary = actions[0] || {
    ...
    reason:
      "All requirements are met. Continue regular monitoring per relationship management guidelines.",
    priority: "Low" as const,
    businessImpact:
      "Maintain relationship health and proactively identify future opportunities.",
  };
